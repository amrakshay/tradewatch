...
def update_config(db: Session, update: ConfigUpdate) -> ConfigRead:
    ...
    if update.scan_time is not None:
        row.scan_time = update.scan_time
    if update.scan_percentage is not None:
        row.scan_percentage = update.scan_percentage
    if update.scan_days is not None:
        row.scan_days = update.scan_days
    if update.alert_check_interval_mins is not None:
        row.alert_check_interval_mins = update.alert_check_interval_mins
    if update.alert_check_start is not None:
        row.alert_check_start = update.alert_check_start
    if update.alert_check_end is not None:
        row.alert_check_end = update.alert_check_end
    if update.dhan_client_id is not None:
        row.dhan_client_id = update.dhan_client_id
    if update.telegram_chat_id is not None:
        row.telegram_chat_id = update.telegram_chat_id
    ...
    if (
        update.alert_check_interval_mins
        and update.alert_check_interval_mins != old["alert_check_interval_mins"]
