...
## Goal
Implement TelegramService: send alert notifications and a test message. Uses HTML parse mode to avoid fragility of Markdown with special characters in stock symbols or user notes.
...
Every user-controlled string (symbol name, signal date, timestamps from config) must be passed through `html.escape()` before embedding in the HTML message. This prevents any `<`, `>`, `&` characters from breaking Telegram's HTML parser.

Stock symbols from NSE are clean ASCII, but notes or custom values entered by the user in the UI may contain arbitrary text. Always escape.

## Done When
- `send_test_message()` sends a visible message to the configured Telegram chat
- `send_alert_triggered(...)` sends a correctly formatted notification with HTML bold elements
- If bot_token or chat_id is empty, methods return `False` without raising exceptions
- `reinit_telegram_service(db)` refreshes credentials after a config update
